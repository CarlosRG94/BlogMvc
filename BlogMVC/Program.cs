...
//Configuracion IOptions para openAI
builder.Services.AddOptions<ConfiguracionesIA>()
    .Bind(builder.Configuration.GetSection(ConfiguracionesIA.Seccion))
    .ValidateDataAnnotations()
    .ValidateOnStart();

//Registrar cliente OpenAI con dependencia de configuración
builder.Services.AddScoped(sp =>
{
    var configuracionesIA = sp.GetRequiredService<IOptions<ConfiguracionesIA>>();
    return new OpenAIClient(configuracionesIA.Value.LlaveOpenAI);//devuelve una instancia de OpenAIClient configurada con la llave de la configuración
});

//Habilita blazor server
...
builder.Services.AddScoped<IAnalisisSentimientos, AnalisisSentimientosAPI>();

builder.Services.AddHttpClient();

//Registra servicio que se ejecuta en segundo plano durante todo el ciclo de vida de la aplicación
...
//LoginPath y AccessDeniedPath se configuran para redirigir a /usuarios/login cuando se necesita autenticación o cuando el acceso está denegado
...
//Segunda configuración de Blazor (detallar errores en circuitos)
builder.Services.AddServerSideBlazor()
